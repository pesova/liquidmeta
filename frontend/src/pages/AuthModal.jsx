...
const IconLock       = () => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>);
const IconBuilding   = () => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>);
...
const IconArrowRight = () => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>);
...
const IconLoader     = () => (<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="am-spin"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>);
...
const Field = ({ icon:Icon, label, type="text", placeholder, value, onChange, name, required }) => {
  ...
  return (
    <div className="am-field">
      {label && <label className="am-label">{label}{required && <span className="am-req">*</span>}</label>}
      <div className="am-input-wrap">
        {Icon && <span className="am-input-icon"><Icon /></span>}
        <input
          className="am-input"
        ...
        />
        {isPw && (
          <button type="button" className="am-eye" onClick={() => setShow(s => !s)}>
...
const UploadField = ({ label, name, value, onChange, hint }) => {
  ...
  return (
    <div className="am-field">
      <label className="am-label">{label}<span className="am-req">*</span></label>
      <div className={`am-upload${value?" done":""}`} onClick={() => ref.current.click()}>
        <input ref={ref} type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display:"none" }}
          onChange={e => { if(e.target.files[0]) onChange(name, e.target.files[0]); }}/>
        <span className="am-upload__icon">{value ? <IconCheck /> : <IconUpload />}</span>
        <span className="am-upload__text">{value ? value.name : `Upload ${label}`}</span>
  ...
  <div className="am-roles">
    {[
      ...
      <button key={r.val} type="button"
        ...
        {value===r.val && <span className="am-role__check"><IconCheck /></span>}
      </button>
    ))}
  </div>
);
...
const StepDots = ({ current, total }) => (
  <div className="am-dots">
    {Array.from({ length: total }).map((_,i) => (
      <div key={i} className={`am-dot${i===current-1?" active":""}${i<current-1?" done":""}`} />
    ))}
  </div>
);
  ...
  <div className={`am-feedback am-feedback--${type}`}>
    ...
    {type==="success" && (
      <div className="am-feedback__stars">
        {[0,1,2].map(i => <span key={i} className="am-feedback__star" style={{"--d":`${i*.15}s`}}><IconStar/></span>)}
      </div>
    )}
    ...
    <button className="am-feedback__btn" onClick={onContinue}>
      {type==="success"
        ? <><span>{role==="vendor" ? "Go to Dashboard" : "Start Shopping"}</span><IconArrowRight /></>
        : <span>Try Again</span>
      }
    </button>
  </div>
...
export default function AuthModal({ isOpen, onClose, defaultTab="login" }) {
  ...
  useEffect(() => {
    ...
    setForgotPw(false); setForgotSent(false); setForgotEmail("");
    setForm(f => ({ ...f, fullName:"", email:"", phone:"", password:"",
      role:"buyer", businessName:"", cacNumber:"", address:"", idType:"",
      cacDocument:null, idDocument:null }));
  }, [tab, isOpen]);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setFeedback({
        ...
        name: user?.name || form.email, role,
        message: role === "vendor" ? "Heading to your vendor dashboard." : "Ready to start shopping!"
      });
    } catch (err) {
      setFeedback({ type: "error", title: "Login Failed", message: err.message || "Invalid email or password." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStep2 = async (e) => {
    ...
    try {
      ...
      setFeedback({
        type: "success", title: "Vendor Account Created!", name: form.businessName, role: "vendor",
        message: "Check your email to verify your account. We'll review your details shortly."
      });
    } catch (err) {
      setFeedback({ type: "error", title: "Registration Failed", message: err.message || "Something went wrong." });
    } finally {
  ...
  return (
    <div className="am-overlay" onClick={e => e.target===e.currentTarget && !feedback && onClose()}>
      <div className="am-modal">
        {feedback && (
          <FeedbackScreen {...feedback} onContinue={handleFeedbackContinue} />
        )}

        <button className="am-close" onClick={onClose}><IconX /></button>
        ...
        {forgotPw ? (
          <div className="am-body">
            <button className="am-back" onClick={() => setForgotPw(false)}><IconArrowLeft /> Back</button>
            {forgotSent ? (
              <div className="am-forgot-sent">
                <div className="am-forgot-sent__icon"><IconMail /></div>
                <h3>Check Your Email</h3>
                <p>We sent a reset link to <strong>{forgotEmail}</strong></p>
                <button className="am-btn am-btn--primary" onClick={() => { setForgotPw(false); }}>Back to Login</button>
              </div>
            ) : (
              <>
                <h2 className="am-title">Reset Password</h2>
                <p className="am-sub">Enter your email and we'll send a reset link</p>
                <form onSubmit={handleForgot} className="am-form">
                  <Field icon={IconMail} name="forgotEmail" placeholder="your@email.com" type="email"
                    value={forgotEmail} onChange={(_,v) => setForgotEmail(v)} required />
                  <button type="submit" className="am-btn am-btn--primary" disabled={loading}>
                    {loading ? <><IconLoader />Sending...</> : "Send Reset Link"}
                  </button>
                </form>
              </>
            )}
        ...
        /* ── VENDOR STEP 2 ── */
          ...
          <div className="am-body">
            <button className="am-back" onClick={() => setStep(1)}><IconArrowLeft /> Back</button>
            <StepDots current={2} total={2} />
            <h2 className="am-title">Business Details</h2>
            <p className="am-sub">Help us verify your business</p>
            <form onSubmit={handleStep2} className="am-form">
              ...
              <div className="am-field">
                <label className="am-label">ID Type<span className="am-req">*</span></label>
                <div className="am-input-wrap">
                  <span className="am-input-icon"><IconShield /></span>
                  <select className="am-input am-select" value={form.idType} onChange={e => set("idType",e.target.value)} required>
              ...
              <UploadField label="CAC Document"  name="cacDocument" value={form.cacDocument} onChange={set} hint="PDF, JPG or PNG" />
              <UploadField label="ID Document"   name="idDocument"  value={form.idDocument}  onChange={set} hint="Clear photo of your ID" />
              <button type="submit" className="am-btn am-btn--primary" disabled={loading}>
        ...
        /* ── SIGNUP STEP 1 ── */
          ...
          <div className="am-body">
            ...
            <form onSubmit={handleStep1} className="am-form">
              ...
              <Field icon={IconMail}  label="Email"         name="email"    placeholder="you@example.com"    value={form.email}    onChange={set} type="email" required />
              <Field icon={IconPhone} label="Phone"         name="phone"    placeholder="+234 800 000 0000"  value={form.phone}    onChange={set} required />
              ...
              <button type="submit" className="am-btn am-btn--primary" disabled={loading}>
                {loading ? <><IconLoader />Processing...</>
                  : form.role==="vendor" ? "Continue →" : "Create Account"}
              </button>
            </form>
            <p className="am-switch">Already have an account? <button onClick={() => setTab("login")}>Log in</button></p>
          </div>
        ...
        /* ── LOGIN ── */
          ...
          <div className="am-body">
            ...
            <div className="am-tabs">
              ...
              <button className="am-tab" onClick={() => setTab("signup")}>Sign Up</button>
            </div>
            <form onSubmit={handleLogin} className="am-form">
              ...
              <div className="am-forgot-link">
                <button type="button" onClick={() => setForgotPw(true)}>Forgot password?</button>
              </div>
              <button type="submit" className="am-btn am-btn--primary" disabled={loading}>
                {loading ? <><IconLoader />Logging in...</> : "Log In"}
              </button>
            </form>
            <p className="am-switch">No account? <button onClick={() => setTab("signup")}>Sign up free</button></p>
          </div>
        )}
